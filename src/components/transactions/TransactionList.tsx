...
import { CheckCircle2, Clock, Trash2, ArrowRightLeft, CreditCard, Wallet } from 'lucide-react';
            ...
            <div className="flex items-center gap-3 shrink-0 ml-3">
              <div className="text-right">
                ...
                <button
                  ...
                  title={tx.paid ? 'Transação confirmada/paga' : 'Transação pendente'}
                >
                  {tx.paid ? (
                    <>
              ...
              <button
                onClick={() => {
                  if (confirm(`Deseja excluir a transação "${tx.description}"?`)) {
                    deleteTransaction(tx.id);
                  }
                }}
                className="opacity-0 group-hover:opacity-100 p-2 text-slate-500 hover:text-rose-400 hover:bg-slate-800 rounded-xl transition-all"
